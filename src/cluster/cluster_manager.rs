...
impl ClusterManager {
    ...
    pub fn dynamic(
        ...
        let span = span!(Level::INFO, source = "cluster::ClusterManager");
            ...
            loop {
                tokio::select! {
                    update = cluster_updates_rx.recv() => {
                        match update {
                            Some(update) => {
                                debug!("Received a cluster update.");
                                cluster_manager.write().update(Self::process_cluster_update(&metrics, update));
                            }
                            None => {
                                warn!( "Exiting cluster update receive loop because the sender dropped the channel.");
                                return;
                            }
                        }
                    }
                    _ = shutdown_rx.changed() => {
                        debug!( "Exiting cluster update receive loop because a shutdown signal was received.");
                        return;
                    },
                }
            }
